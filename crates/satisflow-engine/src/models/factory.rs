...
use crate::models::{
    ...
    raw_input::RawInput, FactoryId, Item, LogisticsId, PowerGeneratorId, ProductionLineId,
    RawInputId,
};
...
pub struct Factory {
    ...
    pub production_lines: HashMap<ProductionLineId, ProductionLine>,
    pub raw_inputs: HashMap<RawInputId, RawInput>, // Raw resource extraction sources
    pub power_generators: HashMap<PowerGeneratorId, PowerGenerator>, // Power generation systems
    pub items: HashMap<Item, f32>,                 // Inventory of items in the factory
}

impl Factory {
    pub fn new(id: FactoryId, name: String, description: Option<String>) -> Self {
        Self {
            id,
            name,
            description,
    ...
    pub fn remove_raw_input(&mut self, id: RawInputId) -> Option<RawInput> {
        self.raw_inputs.remove(&id)
    }

    /// Get a reference to a raw input by ID
    pub fn get_raw_input(&self, id: RawInputId) -> Option<&RawInput> {
        self.raw_inputs.get(&id)
    }

    /// Get a mutable reference to a raw input by ID
    pub fn get_raw_input_mut(&mut self, id: RawInputId) -> Option<&mut RawInput> {
    ...
    pub fn remove_power_generator(&mut self, id: PowerGeneratorId) -> Option<PowerGenerator> {
        self.power_generators.remove(&id)
    }

    /// Get a reference to a power generator by ID
    pub fn get_power_generator(&self, id: PowerGeneratorId) -> Option<&PowerGenerator> {
        self.power_generators.get(&id)
    }

    /// Get a mutable reference to a power generator by ID
    pub fn get_power_generator_mut(&mut self, id: PowerGeneratorId) -> Option<&mut PowerGenerator> {
    ...
    pub fn calculate_item(&mut self, logistics_lines: &HashMap<LogisticsId, LogisticsFlux>) {
        self.items.clear();
        // Add all inputs from logistics input lines
        for line in logistics_lines
            .iter()
            .filter(|(_k, v)| v.to_factory == self.id)
        {
        ...
        for line in logistics_lines
            .iter()
            .filter(|(_k, v)| v.from_factory == self.id)
        {
            for itemflow in &line.1.get_items() {
                *self.items.entry(itemflow.item).or_insert(0.0) -= itemflow.quantity_per_min;
            }
...
mod tests {
    ...
    fn test_add_raw_input() {
        let mut factory = Factory::new(uuid_from_u64(1), "Test Factory".into(), None);
        ...
        assert_eq!(factory.get_raw_input(uuid_from_u64(1)), Some(&raw_input));
    ...
    fn test_remove_raw_input() {
        ...
        let removed = factory.remove_raw_input(uuid_from_u64(1));
    ...
    fn test_remove_nonexistent_raw_input() {
        ...
        let removed = factory.remove_raw_input(uuid_from_u64(999));
    ...
    fn test_get_raw_input() {
        ...
        let retrieved = factory.get_raw_input(uuid_from_u64(1));
    ...
    fn test_get_raw_input_mut() {
        ...
        if let Some(input) = factory.get_raw_input_mut(uuid_from_u64(1)) {
    ...
    fn test_oil_extractor_with_different_purities() {
        let mut factory = Factory::new(uuid_from_u64(1), "Oil Factory".into(), None);
    ...
    fn test_all_extractor_types() {
        ...
        let resource_well = RawInput::new_resource_well(
            uuid_from_u64(4),
            Item::NitrogenGas,
            pressurizer,
            extractors,
    ...
    fn test_add_coal_power_generator() {
        let mut factory = Factory::new(uuid_from_u64(1), "Power Factory".into(), None);
    ...
    fn test_add_fuel_power_generator_with_turbofuel() {
        let mut factory = Factory::new(uuid_from_u64(1), "Advanced Power Factory".into(), None);
    ...
    fn test_add_geothermal_power_generator() {
        let mut factory = Factory::new(uuid_from_u64(1), "Geothermal Factory".into(), None);
    ...
    fn test_add_nuclear_power_generator() {
        let mut factory = Factory::new(uuid_from_u64(1), "Nuclear Factory".into(), None);
    ...
    fn test_remove_power_generator() {
        ...
        let removed = factory.remove_power_generator(uuid_from_u64(1));
    ...
    fn test_remove_nonexistent_power_generator() {
        ...
        let removed = factory.remove_power_generator(uuid_from_u64(999));
        assert_eq!(removed, None);
    }
    ...
    fn test_get_power_generator() {
        ...
        let retrieved = factory.get_power_generator(uuid_from_u64(1));
    ...
    fn test_get_power_generator_mut() {
        ...
        // Get mutable reference and modify
        if let Some(generator) = factory.get_power_generator_mut(uuid_from_u64(1)) {
    ...
    fn test_multiple_power_generators() {
        let mut factory = Factory::new(uuid_from_u64(1), "Mixed Power Factory".into(), None);
        ...
        // Add fuel generators
        let mut fuel_gen = PowerGenerator::new(uuid_from_u64(2), GeneratorType::Fuel, Item::Fuel)
            .expect("Should create valid fuel generator");
    ...
    fn test_power_balance_with_consumption() {
        let mut factory = Factory::new(uuid_from_u64(1), "Balanced Factory".into(), None);
    ...
    fn test_power_balance_deficit() {
        let mut factory = Factory::new(uuid_from_u64(1), "Deficit Factory".into(), None);
    ...
    fn test_factory_with_all_power_generator_types() {
        let mut factory = Factory::new(uuid_from_u64(1), "All Power Types Factory".into(), None);
        ...
        // Coal generator
        let mut coal_gen = PowerGenerator::new(uuid_from_u64(2), GeneratorType::Coal, Item::Coal)
            .expect("Should create valid coal generator");
        let coal_group = GeneratorGroup::new(1, 100.0).expect("Should create valid group");
        coal_gen.add_group(coal_group).expect("Should add group");

        // Fuel generator
        let mut fuel_gen = PowerGenerator::new(uuid_from_u64(3), GeneratorType::Fuel, Item::Fuel)
            .expect("Should create valid fuel generator");
        let fuel_group = GeneratorGroup::new(1, 100.0).expect("Should create valid group");
        fuel_gen.add_group(fuel_group).expect("Should add group");

        // Nuclear generator
        let mut nuclear_gen = PowerGenerator::new(
            uuid_from_u64(4),
            GeneratorType::Nuclear,
            Item::UraniumFuelRod,
        )
        ...
        let mut geo_gen = PowerGenerator::new_geothermal(uuid_from_u64(5));
