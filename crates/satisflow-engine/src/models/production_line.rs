...
impl ProductionLineRecipe {
    ...
    pub fn add_machine_group(&mut self, group: MachineGroup) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (item, rate) in recipe_info.outputs.iter() {
            for group in &self.machine_groups {
                ...
                if group.2 > 0 {
                    ...
                    let sommersloop_multiplier = 1.0 + (group.2 as f32 / recipe_info.machine.max_sommersloop() as f32);
...
impl ProductionLine for ProductionLineBlueprint {
    ...
    fn total_machines(&self) -> u32 {
        self.production_lines.iter().map(|line| line.total_machines()).sum()
    }

    fn total_sommersloop(&self) -> u32 {
        self.production_lines.iter().map(|line| line.total_sommersloop()).sum()
    }
    ...
    fn total_power_consumption(&self) -> f32 {
        self.production_lines.iter().map(|line| line.total_power_consumption()).sum()
    }
}
...
mod tests {
    ...
    fn test_production_line_creation_empty() {
        let production_line = ProductionLineRecipe::new(1, "Test Line".to_string(), None, Recipe::AILimiter);
    ...
    fn test_add_machine_group() {
        let mut production_line = ProductionLineRecipe::new(1, "Test Line".to_string(), None, Recipe::AILimiter);
        let machine_group = MachineGroup::new(5, 150.0, 2);
        production_line.add_machine_group(machine_group).expect("Invalide group");
    ...
    fn test_total_machines() {
        let mut production_line = ProductionLineRecipe::new(1, "Test Line".to_string(), None, Recipe::IronIngot);
        production_line.add_machine_group(MachineGroup::new(4,100.0,0)).expect("Invalid group");
        assert!(production_line.total_machines() == 4);
        assert_eq!(production_line.output_rate(), vec![(Item::IronIngot, 120.0)]);
    ...
    #[test]
    #[should_panic(expected = "Cannot add machine group with more sommersloop than the machine type allows")]
    ...
    fn test_half_somersloop_for_power() {
        let mut production_line = ProductionLineRecipe::new(1, "Test Line".to_string(), None, Recipe::IronIngot);
        production_line.add_machine_group(MachineGroup::new(2,100.0,1)).expect("Invalid group");
