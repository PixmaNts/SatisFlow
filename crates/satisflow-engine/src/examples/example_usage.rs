...
use crate::{
    examples::factory_example::create_sample_factory_setup,
    models::logistics::Transport,
};
...
pub fn run_factory_demo() {
    ...
    for (id, factory) in &engine.factories {
        println!("  Factory {}: {} ({} production lines, {} power generators)", 
            id, factory.name, factory.production_lines.len(), factory.power_generators.len());
    }
    println!();
    ...
    for (id, logistics) in &engine.logistics_lines {
        println!("  Logistics {}: {} -> {} ({})", 
            id, logistics.from_factory, logistics.to_factory, 
    ...
    for item in &key_items {
        if let Some(quantity) = global_items.get(item) {
            let status = if *quantity > 0.0 { "Surplus" } else if *quantity < 0.0 { "Deficit" } else { "Balanced" };
    ...
    for factory_stat in &power_stats.factory_stats {
        let balance_status = if factory_stat.balance > 0.0 { "Surplus" } 
                           else if factory_stat.balance < 0.0 { "Deficit" } 
                           else { "Balanced" };
        println!("  {}: {:.1} MW gen, {:.1} MW cons, {:.1} MW ({})", 
            factory_stat.factory_name, factory_stat.generation, 
            factory_stat.consumption, factory_stat.balance, balance_status);
    }
