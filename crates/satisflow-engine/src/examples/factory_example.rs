...
use crate::{
    models::{
        logistics::{
            Bus, Conveyor, ConveyorSpeed, Pipeline, PipelineCapacity, Train, TransportType,
            TruckTransport, Wagon, WagonType,
        },
...
fn setup_northern_forest_factory(engine: &mut SatisflowEngine, factory_id: FactoryId) {
    ...
    if let Some(factory) = engine.get_factory_mut(factory_id) {
        factory
            .add_raw_input(iron_ore)
            .expect("Should add iron ore input");
        factory
            .add_raw_input(copper_ore)
            .expect("Should add copper ore input");
        factory
            .add_raw_input(limestone)
            .expect("Should add limestone input");
    }
...
fn setup_oil_refinery_factory(engine: &mut SatisflowEngine, factory_id: FactoryId) {
    ...
    if let Some(factory) = engine.get_factory_mut(factory_id) {
        factory
            .add_raw_input(crude_oil)
            .expect("Should add crude oil input");
        factory
            .add_raw_input(water)
...
fn setup_steel_mill_factory(engine: &mut SatisflowEngine, factory_id: FactoryId) {
    ...
    if let Some(factory) = engine.get_factory_mut(factory_id) {
        ...
        factory
            ...
            .expect("Should add limestone input");
        factory
            .add_raw_input(bauxite)
            .expect("Should add bauxite input");
    }
...
fn setup_electronics_lab_factory(engine: &mut SatisflowEngine, factory_id: FactoryId) {
    ...
    if let Some(factory) = engine.get_factory_mut(factory_id) {
        factory
            .add_raw_input(caterium_ore)
            .expect("Should add caterium ore input");
        factory
            .add_raw_input(raw_quartz)
            .expect("Should add quartz input");
        factory
            .add_raw_input(sulfur)
            .expect("Should add sulfur input");
    }
...
fn add_coal_power_generator(
    ...
    generator
        .add_group(group)
        .expect("Should add generator group");
...
mod tests {
    ...
    fn test_sample_factory_creation() {
        ...
        let factory_names: Vec<String> =
            engine.factories.values().map(|f| f.name.clone()).collect();

        assert!(factory_names
            .iter()
            .any(|name| name.contains("Northern Forest")));
        assert!(factory_names
            .iter()
            .any(|name| name.contains("Central Assembly")));
        assert!(factory_names
            .iter()
            .any(|name| name.contains("Oil Refinery")));
        assert!(factory_names.iter().any(|name| name.contains("Steel Mill")));
        assert!(factory_names
            .iter()
            .any(|name| name.contains("Electronics Lab")));

        // Verify production lines exist in each factory
        for factory in engine.factories.values() {
            assert!(
                !factory.production_lines.is_empty(),
                "Factory {} should have production lines",
                factory.name
            );
            assert!(
                !factory.power_generators.is_empty(),
                "Factory {} should have power generators",
                factory.name
            );
        }
        ...
        assert!(
            !engine.logistics_lines.is_empty(),
            "Should have logistics connections"
        );
        assert!(
            engine.logistics_lines.len() >= 8,
            "Should have at least 8 logistics lines"
        );
    }
    ...
    fn test_northern_forest_factory_setup() {
        ...
        // Check raw inputs
        assert_eq!(
            northern_forest.raw_inputs.len(),
            3,
            "Should have 3 raw inputs"
        );

        // Check production lines
        assert_eq!(
            northern_forest.production_lines.len(),
            11,
            "Should have 11 production lines"
        );

        // Check power generators
        assert_eq!(
            northern_forest.power_generators.len(),
            1,
            "Should have 1 power generator"
        );

        // Verify specific production lines exist
        let line_names: Vec<String> = northern_forest
            .production_lines
            .values()
            .map(|line| match line {
    ...
    fn test_oil_refinery_factory_setup() {
        ...
        // Check production lines
        assert_eq!(
            oil_refinery.production_lines.len(),
            7,
            "Should have 7 production lines"
        );

        // Check power generators
        assert_eq!(
            oil_refinery.power_generators.len(),
            1,
            "Should have 1 power generator"
        );

        // Verify specific production lines exist
        let line_names: Vec<String> = oil_refinery
            .production_lines
            .values()
            .map(|line| match line {
                ProductionLine::ProductionLineRecipe(recipe) => recipe.name.clone(),
    ...
    fn test_power_balance_calculation() {
        ...
        assert!(
            !global_items.is_empty(),
            "Should have calculated global items"
        );

        // Check power statistics
        let power_stats = engine.global_power_stats();
        assert!(
            power_stats.total_generation > 0.0,
            "Should have power generation"
        );
        assert!(
            power_stats.total_consumption > 0.0,
        ...
        for factory_stat in &power_stats.factory_stats {
            assert!(
                factory_stat.generation > 0.0,
                "Factory {} should generate power",
                factory_stat.factory_name
            );
            assert!(
                factory_stat.consumption > 0.0,
                "Factory {} should consume power",
                factory_stat.factory_name
            );
        }
    }
    ...
    fn test_item_balance_calculation() {
        ...
        assert!(
            global_items.len() >= 20,
            "Should track at least 20 different items"
        );

        // Should have basic materials
        assert!(
            global_items.contains_key(&Item::IronIngot),
            "Should track Iron Ingot"
        );
        assert!(
            global_items.contains_key(&Item::CopperIngot),
            "Should track Copper Ingot"
        );
        assert!(
            global_items.contains_key(&Item::SteelIngot),
            "Should track Steel Ingot"
        );

        // Should have advanced materials
        assert!(
            global_items.contains_key(&Item::Plastic),
            "Should track Plastic"
        );
        assert!(
            global_items.contains_key(&Item::Computer),
            "Should track Computer"
        );
        assert!(
            global_items.contains_key(&Item::CircuitBoard),
            "Should track Circuit Board"
        );
    }
    ...
    fn test_production_line_power_consumption() {
        ...
        for factory in engine.factories.values() {
            ...
            assert!(
                ...
                "Factory {} should have production power consumption",
                factory.name
            );
        }
    }
    ...
    fn test_factory_interdependencies() {
        ...
        assert!(
            input_factories.len() >= 4,
            "Most factories should receive materials"
        );
        assert!(
            output_factories.len() >= 4,
            "Most factories should send materials"
        );
    }
}
