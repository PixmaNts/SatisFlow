...
use crate::models::{logistics::Transport, Item};
...
pub fn run_test_program() {
    ...
    println!(
        "   Calculated balances for {} different items",
        global_items.len()
    );
    ...
    assert_eq!(
        power_stats.factory_stats.len(),
        5,
        "Should have stats for all factories"
    );
    ...
    for factory in engine.factories.values() {
        ...
        for (line_id, line) in &factory.production_lines {
            ...
            println!(
                "     Line {}: {} inputs, {} outputs, {:.1} MW power",
                line_id,
                inputs.len(),
                outputs.len(),
                power
    ...
    println!(
        "   Total production lines across all factories: {}",
    ...
    for factory in engine.factories.values() {
        ...
        for (input_id, raw_input) in &factory.raw_inputs {
            ...
            assert!(
                raw_input.quantity_per_min > 0.0,
                "Raw input should have positive extraction rate"
            );
        }
    }
    println!(
        "   Total raw inputs across all factories: {}",
        total_raw_inputs
    );
