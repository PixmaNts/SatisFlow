...
use common::{
    create_test_server, create_test_client,
    assertions::*,
    test_data::*,
};
...
async fn test_logistics_crud_operations() {
    ...
    if factory1_response.status().as_u16() == 201 && factory2_response.status().as_u16() == 201 {
        ...
        if create_response.status().as_u16() == 201 {
            ...
            let get_response = client
                .get(&format!("{}/api/logistics/{}", server.base_url, logistics_id))
                .send()
                .await
...
async fn test_cors_headers() {
    ...
    let response = client
        .request(reqwest::Method::OPTIONS, &format!("{}/api/factories", server.base_url))
    ...
    assert!(cors_origin.is_some(), "CORS origin header should be present");
    
    let cors_methods = response.headers().get("Access-Control-Allow-Methods");
    assert!(cors_methods.is_some(), "CORS methods header should be present");
    ...
    let cors_origin = response.headers().get("Access-Control-Allow-Origin");
    assert!(cors_origin.is_some(), "CORS origin header should be present on actual request");
}
