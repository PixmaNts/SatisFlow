//! Satisflow Bridge - FFI bindings for Flutter via flutter_rust_bridge
//!
//! This crate re-exports FFI functions from satisflow-engine with
//! flutter_rust_bridge annotations for Dart binding generation.
...
#[frb]
pub fn ffi_save_to_json(engine: &satisflow_engine::SatisflowEngine) -> FfiResult<String> {
    engine.save_to_json()
}

/// Load the engine state from a JSON string.
#[frb]
pub fn ffi_load_from_json(
    engine: &mut satisflow_engine::SatisflowEngine,
    json: String,
) -> FfiResult<()> {
